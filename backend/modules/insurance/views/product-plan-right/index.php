...
$this->params['breadcrumbs'][] = ['label' => '保障计划', 'url' => ['product-plan/index?product_lang_id=' . $product_lang_id]];
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="../product-plan/index?product_lang_id=<?=$product_lang_id?>"> 保障计划</a></li>
                <li><a href="../product-qa/index?product_lang_id=<?=$product_lang_id?>"> 常见问题</a></li>
            </ul>
            <div class="tab-content">
                <div class="active tab-pane">
                    <div class="box">
            <div class="box-header">
                <?= Html::create(['edit?product_lang_id=' . $product_lang_id . '&product_plan_id=' . $product_plan_id]); ?>
                <span class="btn btn-white" onclick="history.go(-1)">返回保障计划</span>
            </div>
                                ...
                                <?php foreach($model->langs as $langItem):?>
                                    <tr data-pid="<?=$model->id?>" data-id="<?=$langItem->id?>">
                                        ...
                                        <td style="text-align: center;">
                                            <a class="blue" href="edit?product_lang_id=<?=$product_lang_id?>&product_plan_id=<?=$product_plan_id?>&id=<?=$langItem->id?>">编辑</a>
                                        </td>
                                        <?php if ($isFirst): ?>
                                            <td rowspan="<?=$langCount?>" style="text-align: center;">
                                                <a class="red" href="destroy?product_lang_id=<?=$product_lang_id?>&product_plan_id=<?=$product_plan_id?>&id=<?=$model->id?>" onclick="rfDelete(this);return false;">删除</a>
                                            </td>
                                        <?php endif; ?>
                                    </tr>
                                    <?php $isFirst = false;?>
