...
/**
 * This is the model class for table "{{%insurance_cate}}".
 *
 * @property string $id
...
class Cate extends ActiveRecord
{
    ...
    public function getLangs(){
        return $this->hasMany(CateLang::class, ['pid' => 'id']);
    }
    ...
    public function findByLang($lang){
        return CateLang::findOne(['pid' => $this->id, 'lang' => $lang]);
    }

    public function getProductCount(){
        return Product::find()->where(['like', 'cate_id', $this->id])->andWhere(['status' => 1])->count();
    }
    ...
    public static function getMap($id = []){
        $query = Cate::find()->alias('a')
            ->leftJoin(CateLang::tableName() . ' b', 'a.id = b.pid' )
            ->select('a.id, b.name')
        ...
        $list = $query->orderBy('name asc')->asArray()->all();
